...
public class GotoDefinitionFeature implements Feature<TextDocumentPositionParams, Optional<List<Location>>> {
    ...
    private Map<String, List<Token>> directiveBasedDefinitions = new HashMap<>(); // Struct, macro, etc name:node, may
                                                                                  // have collisions across types (ie a
        ...
        return (node -> {
            ...
            if (node.getType().equals(NodeTypes.LABEL_DEFINITION)) {
                ...
                if (directiveNode instanceof EnumNode) {
                    ...
                    enumBody.getChildren().forEach(child -> {
    ...
    public Optional<List<Location>> handle(Project project, TextDocumentPositionParams params) {
        ...
        var element = nodeStream.filter(node -> {
            ...
            if (token != null && token.getPosition() != null) {
                ...
                if (line == position.beginLine && line == position.getEndLine() && column >= position.beginOffset
                    ...
                    return Set.of(NodeTypes.MACRO_CALL, NodeTypes.STRING_EXPRESSION, NodeTypes.MACRO_BODY, // Right now,
                                                                                                           // macro
                                                                                                           // bodies are
                                                                                                           // not parsed
                                                                                                           // but only
                            // tokenized.
                            NodeTypes.IDENTIFIER_EXPRESSION, NodeTypes.OPCODE_ARGUMENT).contains(node.getType());
                }
            }
            return false;
        ...
        if (definition != null) {
            Location loc = new Location(URI.create(this.workspaceRoot + "/" + definition.getFileName()),
                    toRange(definition));
